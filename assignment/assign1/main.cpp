...
void write_courses_offered(std::vector<Course> &all_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::ofstream file("student_output/courses_offered.csv", std::ios_base::ate);
  file << "Title,Number of Units,Quarter\n";
  std::vector<Course> delete_courses{};
  for (int i = 0, n = all_courses.size(); i < n; i++) {
    if (all_courses[i].quarter != "null") {
      delete_courses.push_back(all_courses[i]);
      file << all_courses[i].title << "," << all_courses[i].number_of_units << "," << all_courses[i].quarter << "\n";
    }
  }
  for (int i = 0, n = delete_courses.size(); i < n; i++) {
    delete_elem_from_vector(all_courses, delete_courses[i]);
  }
}
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::ofstream file("student_output/courses_not_offered.csv", std::ios_base::ate);
  file << "Title,Number of Units,Quarter\n";
  for (int i = 0, n = unlisted_courses.size(); i < n; i++) {
    file << unlisted_courses[i].title << "," << unlisted_courses[i].number_of_units << "," << unlisted_courses[i].quarter << "\n";
  }
}
